...
class SAC(nn.Module):
    ...
    def _sumo_cmd(self, cfg_or_env):
        cfg = cfg_or_env.cfg if hasattr(cfg_or_env, "cfg") else cfg_or_env.simulator
        log_dir = os.path.join("saved_files", "sumo_output", getattr(cfg, "city", "sumo"))
        ...
        return [
            ...
            "false",
            "--error-log",
            os.path.join(log_dir, "sumo_error.log"),
            "--message-log",
            os.path.join(log_dir, "sumo_message.log"),
        ]
    ...
    def learn(self, cfg, Dataset=None):
        ...
        for i_episode in epochs:
            ...
            if sim == "sumo":
                sumo_cmd = self._sumo_cmd(self.env)
    ...
    def test(self, test_episodes, env, verbose=True):
        ...
        if sim == "sumo":
            ...
            sumo_cmd = self._sumo_cmd(env)
            sumo_cmd += [
                "--summary-output",
                "saved_files/sumo_output/" + env.cfg.city + "/" + self.agent_name + "_dua_meso.static.summary.xml",
                "--tripinfo-output",
                "saved_files/sumo_output/" + env.cfg.city + "/" + self.agent_name + "_dua_meso.static.tripinfo.xml",
