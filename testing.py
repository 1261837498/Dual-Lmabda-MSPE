...
def _checkpoint_file(checkpoint_path, best=True):
    raw = str(checkpoint_path)
    if raw.endswith(".pth"):
        path = raw
    elif os.path.basename(raw) in {"best", "last"}:
        path = raw + ".pth"
    else:
        suffix = "_best.pth" if best else ".pth"
        path = raw + suffix

    if os.path.isabs(path) or path.startswith("ckpt/") or path.startswith("ckpt\\"):
        return path
    return os.path.join("ckpt", path)
...
def setup_model(cfg, env, parser, device):
    ...
    input_size = getattr(parser, "feature_dim", _cfg_get(cfg.model, "input_size", 0))

    if model_name == "sac" or model_name == "cql":
        from src.algos.sac import SAC
        model = SAC(env=env, input_size=input_size, cfg=cfg.model, parser=parser, device=device).to(device)
        model.load_checkpoint(path=_checkpoint_file(cfg.model.checkpoint_path, best=True))
        return model
    elif model_name == "a2c":
        from src.algos.a2c import A2C
        model = A2C(env=env, input_size=input_size, cfg=cfg.model, parser=parser, device=device).to(device)
        model.load_checkpoint(path=_checkpoint_file(cfg.model.checkpoint_path, best=True))
        return model
    elif model_name == "bc":
        from src.algos.bc import BC
        model = BC(env=env, input_size=input_size, cfg=cfg.model, parser=parser, device=device).to(device)
        model.load_checkpoint(path=_checkpoint_file(cfg.model.checkpoint_path, best=False))
        return model
    else:
...
def test(config):
    ...
    with initialize(config_path="src/config"):
        cfg = compose(config_name="config",
                      overrides=[f"{key}={value}" for key, value in config.items()])  # Load the configuration
    ...
    if os.path.exists(path):
        ...
        no_control_performance = {'reward': no_reb_reward, 'served_demand': no_reb_demand,
                                  'rebalancing_cost': no_reb_cost}
...
def main(cfg: DictConfig):
    ...
    print('Testing...')
    episode_reward, episode_served_demand, episode_rebalancing_cost, episode_inflows = model.test(
        cfg.model.test_episodes, env)

    print('Mean Episode Profit ($): ', np.mean(episode_reward), 'Std Episode Reward: ', np.std(episode_reward))
    print('Mean Episode Served Demand($): ', np.mean(episode_served_demand), 'Std Episode Served Demand: ',
          np.std(episode_served_demand))
    print('Mean Episode Rebalancing Cost($): ', np.mean(episode_rebalancing_cost), 'Std Episode Rebalancing Cost: ',
          np.std(episode_rebalancing_cost))
